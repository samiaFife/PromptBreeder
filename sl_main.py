...
st.markdown(
    f""""PROMPTBREEDER, a general-purpose self-referential self-improvement mechanism that evolves and adapts prompts for a given domain.
...
problem_description = st.text_input(
    "problem description", value="Solve the math word problem, giving your answer as an arabic numeral.", key="pd"
)
...
st.session_state.calls = (
    st.session_state.size * st.session_state.evals + st.session_state.size // 2
) * st.session_state.generations

second_button = st.button(
    f"run for {st.session_state.generations} generations", disabled=(not (st.session_state.size > 0))
)
if second_button:
    st.session_state.population = create_population(
        tp_set=ts_selected_rows["0"].tolist(),
    ...
    co = cohere.Client(
        api_key=st.session_state.COHERE_API_KEY, num_workers=st.session_state.evals, max_retries=5, timeout=60
    )  # override the 2 min timeout with 60s. The APIs performance varies heavily.
